import DartCompetitionBusiness from "../src/Business/DartCompetition/DartCompetitionBusiness"
import { SITUACAO } from "../src/Model/Competition"
import DartCompetitionDataMock from "./mocks/DartCompetitionMocks/DartCompetitionDataMock"

const dartCompetitionBusinessMock = new DartCompetitionBusiness(
    new DartCompetitionDataMock()
)
...
describe('teste ao cadastrar atleta e competição', () => {
    ...
    test("erro ao passar algum input vazio", async () => {
        ...
        try {
            await dartCompetitionBusinessMock.insertDart(input)
    ...
    test("erro ao passar unidade diferente de m", async () => {
        expect.assertions
        const input = ({
            id: "id1",
            competicao: "competicao1",
    ...
    test("erro ao cadastrar competidor já lançou 3 vezes", async () => {
        expect.assertions
        const input = ({
            id: "id2",
        ...
        try {
            ...
            console.log(error.message)
            expect(error.message).toEqual("O atleta já realizou todos os lançamentos!")
        }
    })
...
describe('teste ao finalizar corrida', () => {
    ...
    test("erro ao passar algum input vazio", async () => {
        ...
        try {
            await dartCompetitionBusinessMock.finishDart(input)
...
describe('teste ao pegar Ranking da corrida', () => {
    ...
    test("erro ao passar algum input vazio", async () => {
        ...
        try {
            await dartCompetitionBusinessMock.getRanking(input)
